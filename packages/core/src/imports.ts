...
export function parseCsv(text: string): string[][] {
  return parseCsvRows(text).map(({ row }) => row);
}

function parseCsvRows(text: string): { row: string[]; sourceRow: number }[] {
  const rows: { row: string[]; sourceRow: number }[] = [];
  let row: string[] = []; let cell = ""; let quoted = false; let line = 1; let sourceRow = 1;
  for (let i = 0; i < text.length; i++) { const char = text[i]; const next = text[i + 1];
    if (char === '"' && quoted && next === '"') { cell += '"'; i++; }
      ...
      if (char === '\r' && next === '\n') i++;
      row.push(cell); if (row.some(Boolean)) rows.push({ row, sourceRow });
      row = []; cell = ""; line++; sourceRow = line;
    } else {
      cell += char;
      if (char === '\n' || (char === '\r' && next !== '\n')) line++;
    }
  ...
  row.push(cell); if (row.some(Boolean)) rows.push({ row, sourceRow }); return rows;
}
...
export function previewFeedbackCsv(text: string): ImportPreviewRow[] {
  const [headerRecord, ...rows] = parseCsvRows(text); if (!headerRecord) return [];
  const header = headerRecord.row;
  ...
  return rows.map(({ row, sourceRow }) => { const votes = take(row, "votes", "vote_count"); const title = take(row, "title", "name"); const errors: string[] = [];
  ...
  }; return { sourceRow, item: errors.length ? undefined : item, errors }; });
}
