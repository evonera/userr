...
function usage() {
  console.log("Usage: userr <init|add|doctor|upgrade> [--backend convex|neon|supabase] [--framework next] [--dry-run] [--cwd path]");
}

function isServerPostgresBackend(backend) {
  return backend === "neon" || backend === "supabase";
}
...
function filesFor({ backend, framework }) {
  if (backend !== "convex" && !isServerPostgresBackend(backend)) throw new Error(`Backend '${backend}' is unknown. Available: convex, neon, supabase.`);
  ...
  const backendNotes =
    backend === "convex"
      ...
      : `Install @userr/${backend}, drizzle-orm and pg, apply the ${backend === "supabase" ? "Supabase SQL migrations" : "Postgres migrations"}, wire identify/resolveRole to your auth, then run \`userr add --backend ${backend}\` for the portal pages and API route.`;
  return {
...
function postgresApiRoute({ backend }) {
  const packageName = `@userr/${backend}`;
  return `${OWNERSHIP}import { Pool } from "pg";
import { drizzle } from "drizzle-orm/node-postgres";
import { toNextJsHandler } from "${packageName}";
import * as schema from "${packageName}/schema";
...
function filesForAdd({ backend, framework }) {
  ...
  if (isServerPostgresBackend(backend)) {
    files["app/api/userr/[...path]/route.ts"] = postgresApiRoute({ backend });
  }
...
function doctor() {
  ...
  if (backend === "convex") {
    ...
    checks.push([`@userr/${backend} installed`, Boolean(dependencies[`@userr/${backend}`])]);
