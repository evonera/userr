...
function flowScreens(flow: FeedbackFlow): readonly FlowScreen[] { if (flow.screens?.length && flow.fields?.length) throw new Error("A flow must use fields or screens, not both."); if (flow.screens?.length) return flow.screens; return [{ id: `${flow.id}-form`, fields: flow.fields ?? [] }]; }
export function validateFlow(flow: FeedbackFlow, values: Record<string, unknown>): FlowValidation { const ids = new Set<string>(); const screenIds = new Set<string>(); const screens = flowScreens(flow); if (!screens.length) throw new Error("A flow requires fields or screens."); for (const screen of screens) { if (!screen.id.trim() || screen.id.trim() !== screen.id || screenIds.has(screen.id)) throw new Error("Flow screen ids must be unique, non-empty, and contain no surrounding whitespace."); screenIds.add(screen.id); for (const field of screen.fields) { if (!field.id.trim() || field.id.trim() !== field.id || ids.has(field.id)) throw new Error("Flow field ids must be unique, non-empty, and contain no surrounding whitespace."); ids.add(field.id); } } const activeFields = screens.filter((screen) => screen.when?.(values) ?? true).flatMap((screen) => screen.fields.filter((field) => field.when?.(values) ?? true)); const missing = activeFields.filter((field) => field.required && (!Object.hasOwn(values, field.id) || values[field.id] === undefined || values[field.id] === null || values[field.id] === "")).map((field) => field.id); return { activeFields, missing }; }
