...
export function collectPrivacyMasks(root?: ParentNode): PrivacyMask[] { const scope = root ?? (typeof document === "undefined" ? undefined : document); if (!scope) return []; const sensitive: Element[] = []; const visit = (node: ParentNode) => { for (const element of node.querySelectorAll("*")) { const tokens = (element.getAttribute("autocomplete") ?? "").toLowerCase().split(/\s+/).filter(Boolean); if (element.hasAttribute("data-feedback-mask") || (element instanceof element.ownerDocument.defaultView!.HTMLInputElement && (element.type.toLowerCase() === "password" || tokens.some((token) => token === "current-password" || token === "new-password" || token === "one-time-code" || token.startsWith("cc-"))))) sensitive.push(element); if (element.shadowRoot) visit(element.shadowRoot); } }; visit(scope); return sensitive.flatMap((element) => { const rect = element.getBoundingClientRect(); return Number.isFinite(rect.x) && Number.isFinite(rect.y) && rect.width > 0 && rect.height > 0 ? [{ x: rect.x, y: rect.y, width: rect.width, height: rect.height }] : []; }); }
